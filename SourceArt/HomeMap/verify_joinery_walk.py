"""实际角色穿行衣帽柜前、既有梳妆台入口与卫浴通道；仅起点归位一次。"""
...
movement.stop_movement_immediately();pawn.set_actor_location(unreal.Vector(-820,550,90),False,False)
targets=[[-810,660],[-1000,660],[-1240,660],[-1240,790],[-1240,920],[-1240,790],[-850,790],[-810,1050],[-1180,1120],[-810,1120],[-810,350]]
...
def closure_tick(dt):
    ...
    if closure_result['reached']==len(targets) or now>90:
        ...
        (ROOT/'Docs/Tasks/HomeMap/Reports/joinery_walk.json').write_text(json.dumps(closure_result,indent=2),encoding='utf-8');unreal.unregister_slate_post_tick_callback(closure_handle);return
