"""浅水入口保持 -230 cm；池底坡降到 -398 cm，深水 3.8 m。只执行环境增量。"""
...
def place(n,loc,scale,template=None):
    ...
    return o
# 恢复西侧阳台封边，在正面开 180 cm 直通口；不碰原楼梯。
for cap in ['', '_Cap']:
    r=rows['Balcony_West_Guard'+cap];place(r['name'],r['loc'],r.get('scale',[1,1,1]),'Balcony_East_Guard'+cap)
    z=465 if cap else 407
    place('Balcony_Front_Guard_Right'+cap,[187.5,178,z],[755/1130,1,1],'Balcony_Front_Guard'+cap)
    place('Balcony_Front_Guard'+cap,[-467.5,178,z],[195/1130,1,1])
for n in ['Pool_Side_-440','Pool_Side_440','Pool_End_280','Pool_End_1300']:
    r=rows[n];loc=list(r['loc']);loc[2]=-199;scale=list(r.get('scale',[1,1,1]));scale[2]=398/120;place(n,loc,scale)
def export(o,name):
...
v=[]
for y,z in [(280,-230),(640,-230),(940,-398),(1900,-398)]:
    for x in [-440,440]:v.append((x/100,-y/100,z/100))
for y in [280,640,940,1900]:
    for x in [-440,440]:v.append((x/100,-y/100,-4.28))
f=[]
for i in range(3):
...
for vert in terrain.data.vertices:
    if abs(vert.co.x)<=6.001 and -20.001<=vert.co.y<=-1.999:vert.co.z=-4.7;count+=1
terraintris=export(terrain,'SM_HM_SiteTerrain_DeepPool')
manifest=[]
for name,actor,mat,tris in [('SM_HM_PoolDeepShell','HM_Pool_Base','M_Pool_Mosaic',pooltris),('SM_HM_SiteTerrain_DeepPool','HM_Site_Ground','M_Garden',terraintris)]:
...
print('DEEP_POOL_SOURCE_SAVED',{'deep_cm':380,'pool_triangles':pooltris,'lowered_terrain_vertices':count})
