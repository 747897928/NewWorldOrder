 ...
 */
UCLASS()
class NEWWORLDORDER_API ULyraSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()
...
public:
	/**
	 * 解析没有玩家覆盖时的项目默认语言：Steam 支持语言、系统语言、简体中文回退。
	 * 该结果只用于当前进程，不会写入 GameUserSettings.ini；玩家主动选择的语言仍由设置事务保存。
	 */
	static FString ResolveDefaultCulture();
