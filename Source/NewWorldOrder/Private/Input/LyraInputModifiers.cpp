...
namespace LyraInputModifiersHelpers
{
	...
	static bool IsAiming(const UEnhancedPlayerInput* PlayerInput)
	{
		const APlayerController* PlayerController = PlayerInput
			? Cast<APlayerController>(PlayerInput->GetOuter())
			: nullptr;
		...
		const UAbilitySystemComponent* AbilitySystem = Pawn
			...
			: nullptr;
		return AbilitySystem && AbilitySystem->HasMatchingGameplayTag(FShootGameplayTags::Get().Event_Movement_ADS);
	}
...
FInputActionValue ULyraSettingBasedScalar::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	if (ensureMsgf(CurrentValue.GetValueType() != EInputActionValueType::Boolean, TEXT("Setting Based Scalar modifier doesn't support boolean values.")))
	{
		if (ULyraLocalPlayer* LocalPlayer = LyraInputModifiersHelpers::GetLocalPlayer(PlayerInput))
		{
			...
			if (bApplyTargetingMultiplierWhenADS && LyraInputModifiersHelpers::IsAiming(PlayerInput))
			{
				const double TargetingMultiplier = SharedSettings->GetTargetingMultiplier();
				ScalarToUse *= FMath::Clamp(TargetingMultiplier, 0.01, 10.0);
			}
...
FInputActionValue ULyraInputModifierGamepadSensitivity::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	const bool bUseTargetingSensitivity = TargetingType == ELyraTargetingType::ADS ||
		(bUseADSPresetWhenAiming && LyraInputModifiersHelpers::IsAiming(PlayerInput));
	const ELyraGamepadSensitivity Sensitivity = bUseTargetingSensitivity
