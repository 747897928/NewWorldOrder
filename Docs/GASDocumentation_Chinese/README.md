...
![image-20250502115319003](./README.assets/image-20250502115319003.png)

For Mixed Replication Mode: The OwnerActor's Owner must be the Controller. For Pawns, this is set automatically in PossessedBy (). The PlayerState's Owner is automatically set to the Controller. Therefore, if your OwnerActor is not the PlayerState, and you use Mixed Replication Mode, you must call SetOwner() on the OwnerActor to set its owner to the Controller.
...
![GameplayTag Editor in Project Settings](./README.assets/gameplaytageditor.png)  
...
![Listen for Attribute Change BP Node](./README.assets/attributechange.png) 
...
![Derived Attribute Example](./README.assets/derivedattribute.png)  
...
|Modifier类型|描述|
|:-:|:-:|
|Scalable Float|FScalableFloat结构体可以指向某个横向为变量, 纵向为等级的Data Table, `Scalable Float`会以Ability的当前等级自动读取指定Data Table的某行值(或者在[GameplayEffectSpec](#concepts-ge-spec)中重写的不同等级), 该值还可以进一步被系数处理, 如果没有指定Data Table/Row, 那么就会将其视为1, 因此该系数就可以在所有等级都硬编码为一个值.![ScalableFloat](./README.assets/scalablefloats.png)|
...
![Listen for GameplayEffect Stack Change BP Node](./README.assets/gestackchange-176024809373514.png)
...
![Assigning SetByCaller](./README.assets/setbycaller.png)
...
![Backing Data Attribute Calculation Modifier](./README.assets/calculationmodifierbackingdataattribute.png)
...
![Backing Data Temporary Variable Calculation Modifier](./README.assets/calculationmodifierbackingdatatempvariable.png)
...
![Cost GE With MMC](./README.assets/costmmc.png)
...
![Cooldown GE with SetByCaller](./README.assets/cooldownsbc.png)  
...
![Cooldown GE with MMC](./README.assets/cooldownmmc.png)  
...
![Listen for Cooldown Change BP Node](./README.assets/cooldownchange-176024812010122.png)  
...
![SetByCaller with a GameplayEffectContainer](./README.assets/gecontainersetbycaller.png)  
...
一个简单的`GameplayAbility`流程图: ![Simple GameplayAbility Flowchart](./README.assets/abilityflowchartsimple.png)  

一个更复杂`GameplayAbility`流程图: ![Complex GameplayAbility Flowchart](./README.assets/abilityflowchartcomplex.png)  

复杂的Ability可以使用多个相互交互(激活, 取消等等)的`GameplayAbility`实现.  



下面是把图片里的两个 **GameplayAbility** 流程图转成可复制的纯文本与流程脚本（Mermaid）。你可以任选其一粘贴给其他 AI。
...
![Activate Batched Ability](./README.assets/batchabilityactivate.png)  
...
![Blueprint WaitTargetData AbilityTask](./README.assets/abilitytask.png)  
...
![GameplayCue Triggered from a GameplayEffect](./README.assets/gcfromge.png)  

##### 手动调用

`UGameplayAbility`提供了蓝图节点来`Execute`, `Add`或`Remove` GameplayCue.  

![GameplayCue Triggered from a GameplayAbility](./README.assets/gcfromga.png)  
...
![Reticles in GASShooter](./README.assets/gameplayabilityworldreticle.png)  
...
第一页显示了所有`Attribute`的`CurrentValue`: ![First Page of showdebug abilitysystem](./README.assets/showdebugpage1.png)  

第二页显示了所有应用到你的`持续(Duration)`和`无限(Infinite)GameplayEffect`, 它们的堆栈数, 使用的`GameplayTag`和`Modifier`. ![Second Page of showdebug abilitysystem](./README.assets/showdebugpage2.png)  

第三页显示了所有授予到你的`GameplayAbility`, 无论其是否正在运行, 无论其是否被阻止激活, 和当前正在运行的`AbilityTask`的状态.  ![Third Page of showdebug abilitysystem](./README.assets/showdebugpage3.png)  
...
![Listen for Attributes Changes BP Node](./README.assets/attributeschange.png)  

监听冷却时间修改:  

![Listen for Cooldown Change BP Node](./README.assets/cooldownchange.png)  

监听`GE`堆栈修改:  

![Listen for GameplayEffect Stack Change BP Node](./README.assets/gestackchange.png)  
